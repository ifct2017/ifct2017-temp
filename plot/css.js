...
function setComputedStyle(doc) {
  for(var i=0, I=SELECTOR.length; i<I; i++) {
    for(var elm of doc.querySelectorAll(SELECTOR[i])) {
      if(elm.nodeName==='text' && (CONTENT[i].includes('flex') || (!CONTENT[i].includes('webkit') && !CONTENT[i].includes('moz') && !CONTENT[i].includes('fill') && !CONTENT[i].includes('line') && !CONTENT[i].includes('font') && !CONTENT[i].includes('color')))) continue;
